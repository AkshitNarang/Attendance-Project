...
def generate_qr(lines):
    ...
    for i in range(0, len(lines)):
        ...
        print(name)
        version, level, qr_name = myqr.run(str(name), version=1, level='H', picture='QR_bg.jpg', colorized=False,
                                           contrast=1.6, brightness=1.0, save_name=str(lines[i]+'.bmp'), save_dir=os.getcwd())
